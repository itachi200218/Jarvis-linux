...
# ==============================
user_command_rules_col = db["user_command_rules"]
...
def get_user_disabled_intents(user_id: str):
    if not user_id:
        return []

    rule = user_command_rules_col.find_one({"userId": user_id})
    if not rule:
        return []

    return rule.get("disabledIntents", [])
...
def handle_command(
    ...
    ctx = TAB_MEMORY[chat_id]["context"]
    # ==============================
# 🧠 GLOBAL INTENT DETECTION (ONCE)
# ==============================
    detected_intent, score = find_intent(command)
    
# ==============================
# 🔒 USER-SPECIFIC COMMAND BLOCK
# ==============================
    user_disabled_intents = get_user_disabled_intents(user_id)

    if detected_intent and detected_intent in user_disabled_intents:
        return {
            "reply": "This command is disabled for your account by admin.",
            "intent": "user_blocked",
            "confidence": score
        }

    # ==============================
    # 🔐 ADMIN DISABLED COMMAND CHECK
    # ==============================
    # if detected_intent:
    #     cmd_doc = next(
    #         (c for c in COMMAND_CACHE if c.get("intent") == detected_intent),
    #         None
    #     )

    #     if cmd_doc and cmd_doc.get("disabled") is True:
    #         return {
    #             "reply": "This command is temporarily disabled by admin.",
    #             "intent": "admin_disabled",
    #             "confidence": score
    #         }
...
#    # ==============================
# # 🚫 BLOCK SYSTEM COMMANDS ONLY IN SILENT WINDOWS
# # ==============================
#     if silent and is_command(raw):
#         # detected_intent, score = find_intent(command)

#         if detected_intent in SYSTEM_INTENTS:
#             response = "System commands are disabled here."
#             if not silent:
#                 speak_async(response)
#             return {
#                 "reply": response,
...
#             }
    ...
    # ==============================
  # ==============================
# 📍 SET DEFAULT LOCATION (GLOBAL, SAFE)
# ==============================
    location_value = extract_location_set(raw)

    if location_value:
        detected_intent = "set_location"

        user_disabled_intents = get_user_disabled_intents(user_id)
        if detected_intent in user_disabled_intents:
            return {
                "reply": "This command is disabled for your account by admin.",
                "intent": "user_blocked",
                "confidence": 100
...
# ==============================
    ...
    if media_intent in MEDIA_INTENTS:
        ...
        # 🔒 BLOCK GUEST HERE (THIS WAS MISSING)
        if user_role == "guest":
